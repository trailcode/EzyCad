...
- **Sketch overlapping edges**: Drawing the same line or circle twice, or adding a collinear / same-circle overlap, no longer stacks a second copy. The sketch splits at overlap ends and keeps each atomic piece once. Undo records only the new atomic pieces, so extending a line past an existing collinear edge no longer deletes the original on undo. Fixes an access violation in `TKGeomBase` when a new circle coincided with an existing arc (for example a bone end-cap).
...
- **Curve segment angle**: **Settings -> View presentation** slider (**2** to **20** degrees, default **6**) sets Open CASCADE AIS curve tessellation. Smaller draws smoother arcs and fillets (display only). Persisted as **`gui.curve_deviation_angle_deg`**.

- **Add bone** sketch tool (<kbd>Shift</kbd>+<kbd>U</kbd>): click the two circle centers, then radius at the second center, radius at the first center, and waist width (like other sketch tools). Waist is the minimum neck thickness (offset from mid when end radii differ). Live length dim and a tmp segment for center-to-center, then for each radius, waist, and hole so <kbd>Tab</kbd> can enter exact values (measure segments are not committed). While placing radius 1, snap targets sit on the second circle at the bone-axis-parallel tangents (match radius 2). Options: **Add center nodes**, **Add radius nodes**, and **Add total length nodes** (all default on) plus **Holes** (**None** / **One radius** / **Two radii**). Radius nodes are **Bone A+** / **A-** / **B+** / **B-**; total-length nodes are **Bone A tip** / **Bone B tip**. Waist cutters are tangent to both end circles. Commits the trimmed outline (outer end arcs and inner waist arcs), optional hole circles, and optional permanent **Bone A** / **Bone B** center nodes, then exits to **Sketch inspection** (does not stay in Add bone for another). Remappable as **`mode.add_bone`**.
...
- **File -> Export:** After choosing a format, an **Export units** dialog offers **Inches** or **Millimeters** (defaults: mm for STEP/IGES, inches for STL/PLY). STEP/IGES declare the chosen unit in the file; STL/PLY remain unitless with coordinates in that unit.
...
[Unreleased]: https://github.com/trailcode/EzyCad/compare/v0.4.0...HEAD
[0.4.0]: https://github.com/trailcode/EzyCad/releases/tag/v0.4.0
