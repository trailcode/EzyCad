...
- **STEP import progress**: after choosing a STEP file (and again on **Import into project**), a modal shows stage text and transfer progress. Desktop runs the OCCT transfer on a background thread so the UI stays responsive (Cancel available); metadata collect uses a single Transfer pass instead of parsing the file twice.
