...
- **Align cylinders** (<kbd>J</kbd>): pick two cylindrical faces (first moves, second is fixed), coaxially align, drag axial insert depth, then twist about the shared axis for tooth clocking (or <kbd>Enter</kbd> during depth to skip twist). <kbd>Tab</kbd> edits depth or twist angle by phase. Options **Flip direction** reverses axis sense. Bakes like Move/Rotate (`Shape_geom_delta`); radius mismatch logs a warning but still places. Remappable as **`mode.cyl_align`**.
