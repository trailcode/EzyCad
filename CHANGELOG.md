...
- **STEP import progress**: on **Import into project**, a modal shows stage text and transfer progress. Desktop runs the OCCT transfer on a background thread so the UI stays responsive (Cancel available). The Import dialog no longer shows a file metadata summary (opens immediately after file pick).
