...
**Aliases** (compatibility): global **`view`** is **`ezy.view`**; global **`Shp`** is **`ezy.Shp`**; **`help()`** is **`ezy.help()`**. Flat sketch names on `view` (`curr_sketch_*`, `add_sketch`, `add_edge`, `finish_sketch_edges`) forward to **`ezy.sketch.*`**.
...
| `ezy.view.add_box(ox, oy, oz, w, l, h)`                   | Add a box                                        |
| `ezy.view.add_sphere(ox, oy, oz, r)`                      | Add a sphere                                     |
...
| Method                                           | Purpose                                                                      |
| ------------------------------------------------ | ---------------------------------------------------------------------------- |
| `ezy.sketch.curr_name()`                         | Name of the current sketch                                                   |
| `ezy.sketch.node_count()` / `ezy.sketch.node(i)` | Read nodes `(x, y)` on the current sketch plane                              |
| `ezy.sketch.dim_count()` / `ezy.sketch.dim(i)`   | Read length dimensions                                                       |
| `ezy.sketch.add(plane, offset, base_name)`       | New sketch on `XY`, `XZ`, or `YZ` (`offset` in display units; optional name) |
| `ezy.sketch.add_edge(x1, y1, x2, y2)`            | Add a linear edge to the current sketch                                      |
| `ezy.sketch.finish_edges()`                      | Rebuild closed-face topology after bulk edge import                          |
...
```python
from ezycad_remote import Session

with Session.connect("127.0.0.1", 8765) as s:
    s.ezy.log("hello from remote")
    print(s.ezy.view.sketch_count())
    print(s.view.sketch_count())  # alias
```

Responses include captured `print` / `ezy.log` text, expression `repr` when available, and error text on failure.
