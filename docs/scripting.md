...
| `ezy.set_mode(name)`            | Switch mode by name (`Design_inspection`, `Design_move`, `Design_shaft_align`, `Workbench_move`, `Scale`, ...). Retired names `Normal` / `Sketch_inspection_mode` / `Move` (Design move) / `Rotate` (Design rotate) / `Workbench_scale` / `Shape_shaft_align` still work. |
