...
Sketch arcs, circles, and rounded 3D edges are true curves. The viewer draws them as polylines. **Settings -> View presentation -> Curve segment angle** (`gui.curve_deviation_angle_deg`, **2** to **20** degrees, default **6**) is the maximum angle between consecutive segments. Smaller values look smoother and use more segments. This does not change stored geometry or mesh export deflection.

## Settings file

Background gradient, grid lines, and related **3D appearance** are stored under **`occt_view`** in your settings JSON. **View rotation step**, **zoom scroll scale**, **default 2D view width/height** (New Project framing), **element hover color**, **shape selection color**, and **curve segment angle** are stored under **`gui`**. Full lists of keys: **[usage-settings.md — Settings file reference](usage-settings.md#settings-file-reference)**.
