...
- **Workbench List** - Show or hide the Workbench List pane.
...
6. **Sketch** — Nested sections (open state remembered in `gui.settings_headers`):
   ...
   - **Nodes** — permanent node marker size and origin color; three midpoint rows (line edges, square/rectangle, slot).
   ...
   - **Permanent node annotation size** (scale for **+** markers on the sketch **Origin** and [Add node](usage-sketch.md#add-node-tool) points; see [Sketch origin](usage-sketch.md#sketch-origin)), **Origin marker color**, **Add midpoints to line edges** (default off) / **square/rectangle** (default on) / **slot edges** (straight edges only, default off), **Underlay highlight color**, **Snap guide color (node)** / **(axis)**, **Snap guide line width** (desktop only; slider **0.5** to **8.0**; default **1.0**), **Snap guide mode** (*Traditional*, *Fullscreen*, *Both*, *None*), **All co-axial nodes** (hidden when mode is *None*)
...
- **Extrude sketch face**: under **Extrude**, **Both sides** (default on), **Twist**, and **Material** for the new solid (same document preset as **Normal** mode Options **Material**). With **Twist** on, the first drag sets height, then a second drag sets twist angle about the face center; <kbd>Tab</kbd> / <kbd>Shift+Tab</kbd> enter height / angle. Fast drag preview for dense faces is configured in **Settings -> Sketch -> Appearance** (**Extrude fast preview**); with **Twist**, face copies rotate as well as translate. Other modes that still show **Material** in Options use that same preset when relevant (for example **Sketch from planar face**).
- **Add edge** / **Add node** (and similar): a **Shortcuts** line documents TAB / Shift+TAB typing behavior.
- **Add line edge**: under **Options**, **Add midpoint nodes** ([usage-sketch.md#line-edge-option-add-midpoint-nodes](usage-sketch.md#line-edge-option-add-midpoint-nodes)) and **Place from center** ([usage-sketch.md#line-edge-option-place-from-center](usage-sketch.md#line-edge-option-place-from-center)); each has a **?** link. See also [Line edge Options](usage-sketch.md#line-edge-options). **Add midpoint nodes** is the same control on multi-line, arc, square, both rectangle tools, and slot, because those tools place straight edges; the checkbox **?** opens that same section. The saved rows under **Settings -> Sketch -> Nodes** are separate: **Add midpoints to line edges** (`gui.add_mid_pt_edges`, default **off**; also multi-line and arc), **Add midpoints to square/rectangle** (`gui.add_mid_pt_rect_edges`, default **on**), and **Add midpoints to slot edges** (`gui.add_mid_pt_slot_edges`, straight edges only, default **off**).
- **Add bone**: under **Options**, **Add center nodes**, **Add radius nodes**, and **Add total length nodes** (all default on) and **Holes** (**None**, **One radius**, **Two radii**). Persisted as **`gui.bone_add_center_nodes`**, **`gui.bone_add_radius_nodes`**, **`gui.bone_add_total_length_nodes`**, and **`gui.bone_holes`**. See [Bone Creation Tool](usage-sketch.md#bone-creation-tool).
...
| `show_workbench_list`                 | boolean            | Workbench List pane visible (default **true**).                                                                                                                                                                                                                                                                                                                                                   |
...
| `add_mid_pt_edges`                    | boolean            | **Settings -> Sketch -> Nodes -> Add midpoints to line edges.** Line, multi-line, and arc **Add midpoint nodes** (default **false**). Legacy `add_midpoints_to_linear_edges` is read when this key is absent. See [Line edge option: Add midpoint nodes](usage-sketch.md#line-edge-option-add-midpoint-nodes).                                                                                    |
| `add_mid_pt_rect_edges`               | boolean            | **Add midpoints to square/rectangle** (square and both rectangle tools; default **true**). Same help section as line edges.                                                                                                                                                                                                                                                                       |
| `add_mid_pt_slot_edges`               | boolean            | **Add midpoints to slot edges** (slot straight edges only; default **false**). Same help section as line edges.                                                                                                                                                                                                                                                                                   |
| `bone_add_center_nodes`               | boolean            | Add-bone Options: commit permanent **Bone A** / **Bone B** (default **true**).                                                                                                                                                                                                                                                                                                                    |
| `bone_add_radius_nodes`               | boolean            | Add-bone Options: commit permanent **Bone A+** / **A-** / **B+** / **B-** axis-tangent nodes (default **true**).                                                                                                                                                                                                                                                                                  |
| `bone_add_total_length_nodes`         | boolean            | Add-bone Options: commit permanent **Bone A tip** / **Bone B tip** outer-end nodes (default **true**).                                                                                                                                                                                                                                                                                            |
| `bone_holes`                          | integer            | Add-bone Options holes mode: **0** None, **1** One radius, **2** Two radii (default **0**).                                                                                                                                                                                                                                                                                                       |
| `transform_space`                     | integer            | Move / rotate / scale Options: **0** Local (first selected solid's frame; default), **1** World (global XYZ at that solid's bbox center).                                                                                                                                                                                                                                                         |
...
| `curve_deviation_angle_deg`           | number             | Max AIS curve segment angle in degrees (**2** to **20**; default **6**). Smaller is smoother on-screen (sketch arcs, round solids). Display only; STEP/STL export is unchanged. See [usage-occt-view.md](usage-occt-view.md#curve-display).                                                                                                                                                       |
...
| `extrude_fast_preview`                | boolean            | When **true** (default), extrude uses a face-copy drag preview for faces with more edges than `extrude_fast_preview_edge_threshold`. Settings -> Sketch -> Appearance. See [Extrude](usage.md#extrude-sketch-face-tool-e).                                                                                                                                                                        |
| `extrude_fast_preview_edge_threshold` | integer            | Edge-count threshold for extrude fast preview (**4** to **256**; default **24**).                                                                                                                                                                                                                                                                                                                 |
