...
- **Extrude sketch face**: under **Extrude**, **Both sides**, **Shaded preview** (show the drag preview as a shaded solid instead of the default wireframe; slower on faces with many edges), and **Material** for the new solid (same document preset as **Normal** mode Options **Material**). Other modes that still show **Material** in Options use that same preset when relevant (for example **Sketch from planar face**).
