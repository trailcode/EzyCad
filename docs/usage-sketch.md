...
While you draw or place points in sketch mode, EzyCad helps you align to existing geometry. **Snap guide mode** is listed first in the Options panel; **Snap dist** and **All co-axial nodes** follow (and are hidden when mode is *None*). **Snap guide color**, **Snap guide line width** (desktop only), and the same mode/co-axial toggles are in **Settings -> Sketch** (see [usage-settings.md](usage-settings.md#sketch-tools)).

|                                               |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| --------------------------------------------: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Snap distance**                             | Larger **Snap dist** values let snaps engage from farther away (screen pixels, converted to the sketch plane at the cursor). Hidden when **Snap guide mode** is *None*.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **Snap guides**                               | **Snap guide mode**: *Traditional* (local markers at guide intersections), *Fullscreen* (view-spanning axis lines), *Both*, or *None* (no snap-to-node and no guides). **Snap guide color (node)** is used when both X and Y align to the same vertex; **Snap guide color (axis)** when only one axis aligns. A separate checkbox **All co-axial nodes** (in the sketch Options panel and in Settings; hidden when mode is *None*) enables *global* mode: when on, full horizontal and vertical guide lines + markers are shown for *all* nodes in the current sketch and all other visible sketches (the complete set of co-axial alignments). When off (default), only the closest node per active axis is annotated (classic closest-relative behavior). |
...
| **Overlapping / duplicate edges**             | If a new straight edge or arc lies on the **same line or same circle** as an existing edge and they overlap along a range (including drawing the same edge twice), the sketch keeps each atomic piece **once**. Existing edges split at the overlap ends; the overlapping copy is not added.                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| **Intersection splitting**     | Splits at interior crossings; same-circle overlap is merged (no second copy)                                                                                    |
...
- Adding an arc or circle on top of an existing arc of the **same** circle does not create a second copy of the overlapping part (the rest of the new arc is still added).
...
A live length dimension and rubber-band segment are shown for the two centers, then for each radius, the waist, and hole radii so <kbd>Tab</kbd> can enter an exact value. Those measure segments are temporary (they are not committed as sketch edges).
