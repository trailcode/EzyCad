...
| **Twist**                        | Options **Twist**: after locking height (click or <kbd>Tab</kbd> commit), the height dimension is removed and a temporary angle annotation shows twist in degrees. Drag to rotate the far end about the face center, or <kbd>Shift+Tab</kbd> for an exact angle. With **Both sides**, the mid-plane stays unrotated and each end twists by half the angle in opposite directions |
