...
- **Local frame (solids)** - Each solid has a local `XYZ` frame (default: world-aligned at the bounding-box center; **Z** is the primary axis, **Y** is up). Right-click the name or **M** for:
  - **Show axes** / **Show plane** / **Show up** - toggle 3D annotations (off by default; saved in the `.ezy` file).
  - **Reset frame to bbox** - world-aligned frame at the bbox center (turns **Show axes** on).
  - **Set from planar face...** / **Set from cylindrical face...** - pick a face on that solid; plane sets **Z** = normal, cylinder sets **Z** = axis (origin = solid center projected onto the plane/axis).
