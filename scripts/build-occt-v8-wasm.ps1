# Download and build Open CASCADE Technology (OCCT) V8.0.0 for WebAssembly (Emscripten).
#
...
Write-Host ""
Write-Host "EzyCad wasm configure example (Ninja generator works with OCCT V8):"
Write-Host "  mkdir build_em"
Write-Host "  cd build_em"
Write-Host "  emcmake cmake $((Split-Path -Parent $PSScriptRoot)) -Wno-dev -G Ninja -DOpenCASCADE_DIR=$OpenCascadeDir -DCMAKE_BUILD_TYPE=Release"
Write-Host "  ninja"
