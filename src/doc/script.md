...
| `ezy.get_mode()`                | `GUI::get_mode()` -> mode name string (`c_mode_strs`; Workbench tools are `Workbench_move` etc.; Scale is `Scale`) |
| `ezy.set_mode(name)`            | `GUI::set_mode(mode_from_string(...))` (aliases: `Move` / `Rotate` / `Workbench_scale` / `Shape_shaft_align`)      |
...
| `set_selected(s1, ...)`                  | `set_selected_shps`; list/table ok; no args clears      |
