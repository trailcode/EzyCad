...
| `shp_extrude.h`       | `Shp_extrude`          | Two-click extrude of `Sketch_face_shp`. Live preview caches a height-1 prism and stretches it along the plane normal (`gp_GTrsf` affinity) instead of re-sweeping each move; drag preview is wireframe by default (Options **Shaded preview** switches the live preview to shaded + material), `finalize` bakes the shaded solid; tmp length dimension reused in place; `cancel`; optional both-sides extrude.                                                                                          |
