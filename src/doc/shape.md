...
- Workbench leaves are **links**: `source_id` + local geom `Inverse(trsf_from_frame(source.frame)) * source.Shape()`, instance `gp_Ax3` as placement (`trsf_from_frame` is local-to-world; do not use single-arg `gp_Trsf::SetTransformation`). `add_to_workbench` copies Design nodes; `sync_workbench_links` refreshes after Design geom replace (fillet/chamfer keep `Shape_id`). Design bake of move/rotate does not move instances. Workbench bake updates frame only. `.ezy` `workbench[]` stores ids / `sourceId` / frame (no BREP). Deleting a Design source cascade-deletes its links.
