...
`Occt_view` owns the Design shape list (`m_shps`) and the Workbench instance list (`m_wbk_shps`), constructs all operation objects, routes UI input, and handles JSON save/load for both.
...
- Interactive move, rotate, scale, and cylindrical align with preview transforms.
...
- Design shapes are stored in `Occt_view::m_shps`. Workbench instances live in `m_wbk_shps` (separate list). Access via `get_shapes()` / `get_workbench_shapes()`. Shared `allocate_shape_id()`.
- Workbench leaves are **links**: `source_id` + local geom `Inverse(trsf_from_frame(source.frame)) * source.Shape()`, instance `gp_Ax3` as placement (`trsf_from_frame` is local-to-world; do not use single-arg `gp_Trsf::SetTransformation`). `add_to_workbench` copies Design nodes; `sync_workbench_links` refreshes after Design geom replace. Fillet, chamfer, cut, fuse, common, and cross-section clip keep the edited solid's `Shape_id` (boolean result reuses the first operand). `Shape_replace_delta` does not cascade workbench links for an id that the same delta inserts again. Consumed operands (cut tools, fused-away solids) still cascade-delete their links. User delete of a Design source records those links in `Shape_remove_delta` so undo restores them. Cut, fuse, and common reject Workbench instances (they are placements, not Design BREP). Design bake of move/rotate does not move instances. Workbench bake updates frame only. `unlink_workbench` deep-copies local geom and clears `source_id` so later Design edits do not refresh that instance (undo restores the link). `.ezy` `workbench[]` stores ids / `sourceId` / frame; linked rows omit BREP, unlinked rows (`sourceId` 0) store `geom`. Deleting a Design source cascade-deletes its links; unlinked instances stay.
...
| **Finalize** | `operation_shps_finalize_()` -> `bake_transform_into_geometry()` (Workbench: frame only) | New `Shp`; `delete_operation_shps_()`; `add_shp_()` |
| **Cancel**   | `operation_shps_cancel_()` -> `SetLocalTransformation(placement_trsf())`                 | N/A                                                 |
...
shp_transform.h / shp_transform.cpp      Local/World axes and constrained translation for move/rotate/scale
...
| `shp_move.h`          | `Shp_move`             | Drag on view plane; `Transform_space` Local/World; axis constraints (`Move_options`); Tab distance entry; finalize bakes translation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `shp_rotate.h`        | `Shp_rotate`           | Rotate about view axis or Local/World X/Y/Z through the current pivot; angle Tab entry; axis/center AIS guides (Topmost). Axis can be set before the first drag. View-to-object axis and the axis-vs-view drag plane are captured on the first mouse sample so orbit/pan does not retarget a stale anchor. Mid-drag Local/World re-seeds the pivot and re-baselines the mouse anchor so the angle does not jump.                                                                                                                                                                                                                         |
| `shp_scale.h`         | `Shp_scale`            | Uniform scale from Local frame origin or World bbox center vs mouse distance; clamped factor 0.01..100. Mid-drag Local/World re-seeds the pivot and re-baselines distance so the factor does not jump.                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `shp_transform.h`     | helpers                | `transform_axes_for` / `transform_translation` shared by move, rotate, scale.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `shp_cyl_align.h`     | `Shp_cyl_align`        | Pick two cylindrical faces (first moves); map cylinder axes through AIS `LocalTransformation` (Workbench instance pose; identity on Design); coaxial `cyl_align_trsf` (default acute sense; Options flip forces 180°); drag axial depth; Options **Clock rotation** (default off) then LMB / Shift+Tab about shared axis; bake like Move.                                                                                                                                                                                                                                                                                                |
| `shp_set_frame.h`     | `Shp_set_frame`        | Shape List (`Shape_set_frame`) or Workbench List (`Workbench_set_frame`): pick planar or cylindrical face. Design writes `get_frame()`; Workbench writes `get_local_frame()` so instance pose stays put. Enables Show axes.                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| ---------------------------------------------- | -------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- | ------------------------------------------- |
| `Design_move` / `Workbench_move`               | `shp_move().move_selected`       | `shp_move().finalize`                                                                                             | `shp_move().show_dist_edit` (`gui_mode`)                               | `shp_move().cancel` -> parent idle          |
| `Design_rotate` / `Workbench_rotate`           | `shp_rotate().rotate_selected`   | `shp_rotate().finalize`                                                                                           | `shp_rotate().show_angle_edit`                                         | `shp_rotate().cancel` -> parent idle        |
| `Mode::Scale`                                  | `shp_scale().scale_selected`     | `shp_scale().finalize`                                                                                            | --                                                                     | `shp_scale().cancel` -> `Design_inspection` |
| `Design_shaft_align` / `Workbench_shaft_align` | `drag_depth` / `drag_twist`      | Pick faces; LMB depth->clock if Clock rotation on; LMB/Enter finalize                                             | Tab depth; Shift+Tab clock/angle; Enter finalize                       | `shp_cyl_align().cancel` -> parent idle     |
