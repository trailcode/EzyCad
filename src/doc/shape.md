...
| `shp_cross_section.h`   | `Shp_cross_section`          | Shared `BRepAlgoAPI_Section` preview for the selection (first-shape local XY/XZ/YZ orientation, selection-bbox center + offset); temporary cyan section wires plus one translucent yellow plane/normal annotation; optional AIS `Graphic3d_ClipPlane` (**Hide back side**); **Clip** half-space-commons each solid and replaces the inputs. Desktop preview sections solids on a worker pool (WASM stays single-threaded); bbox cull skips obvious misses. |
