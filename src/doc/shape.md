...
| `Mode::Sketch_face_extrude`   | `sketch_face_extrude(..., true)` | 1st click: pick face; 2nd click (view): `finalize_sketch_extrude_` if active (`GUI` skips `on_left_click_` so the same press does not re-pick) | `dimension_input` / `on_enter` refresh preview | `m_shp_extrude.cancel`                 |
