...
After applying a delta or reloading a snapshot, `Occt_view` restores a UI mode via `GUI::set_mode`. Stored `Design_move` / `Design_rotate` / `Design_shaft_align` / `Workbench_move` / `Workbench_rotate` / `Workbench_shaft_align` / `Scale` / `Shape_set_frame` are mapped to their parent mode (`GUI::parent_mode_of`) so undo/redo does not re-enter a free-drag tool or face-pick with no target. Other stored modes are restored as-is. If the restored mode is `Sketch_inspection`, the sketch list pane is shown again.
