...
void GUI::open_add_bone_dialog()
{
  m_open_add_bone_popup   = true;
  m_add_bone_dialog_open  = true;
}

void GUI::dismiss_add_bone_dialog()
{
  m_open_add_bone_popup  = false;
  m_add_bone_dialog_open = false;
}

void GUI::add_bone_dialog_()
{
  if (m_open_add_bone_popup)
  {
  ...
  if (!m_add_bone_dialog_open)
    return;

  if (!ImGui::BeginPopupModal("Add bone", &m_add_bone_dialog_open, ImGuiWindowFlags_AlwaysAutoResize))
  {
    if (!m_add_bone_dialog_open && m_view)
      m_view->curr_sketch().cancel_elm();
  ...
  ImGui::Text("Values in project units (%s).", m_view->project_unit_suffix());
  ImGui::Spacing();
  if (ImGui::BeginTable("Add bone##table", 2, ImGuiTableFlags_SizingStretchProp))
  {
    table_row_input_double_("Radius 1", "##bone_r1", &m_add_bone_r1);
    table_row_input_double_("Radius 2", "##bone_r2", &m_add_bone_r2);
    table_row_input_double_("Cut radius", "##bone_cut", &m_add_bone_cut_radius);
    if (ImGui::IsItemDeactivatedAfterEdit())
      m_add_bone_drive = Bone_drive::Cut_radius;
    table_row_input_double_("Waist", "##bone_waist", &m_add_bone_waist);
    if (ImGui::IsItemDeactivatedAfterEdit())
      m_add_bone_drive = Bone_drive::Waist;
    ImGui::EndTable();
  }

  const double scale = m_view->get_display_to_model_scale();
  const double r1    = m_add_bone_r1 * scale;
  const double r2    = m_add_bone_r2 * scale;
  const double cut   = m_add_bone_cut_radius * scale;
  const double waist = m_add_bone_waist * scale;
  m_view->curr_sketch().update_bone_preview(r1, r2, cut, waist, m_add_bone_drive);

  if (const std::optional<Bone_geom> g = m_view->curr_sketch().last_bone_preview_geom())
  {
    const double inv = 1.0 / scale;
    if (m_add_bone_drive == Bone_drive::Waist)
      m_add_bone_cut_radius = g->cut_radius * inv;
    else
      m_add_bone_waist = g->waist * inv;
  }

  const bool can_add = m_add_bone_r1 > 0 && m_add_bone_r2 > 0 && m_add_bone_cut_radius > 0 && m_add_bone_waist > 0;
  ImGui::Spacing();
  if (ImGui::Button("Add") && can_add)
  {
    if (m_view->curr_sketch().commit_pending_bone(r1, r2, cut, waist, m_add_bone_drive))
    {
      ImGui::CloseCurrentPopup();
      m_add_bone_dialog_open = false;
    }
