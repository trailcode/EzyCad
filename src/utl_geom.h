...
/// Cylinder axis and radius from a cylindrical face (location applied).
struct Cyl_face_info
{
  gp_Ax1 axis;
  double radius{0};
};
...
/// \a axial_offset then slides along the fixed direction. Face cylinder dirs are
/// unsigned: the default sense is the smaller rotation (acute). \a flip forces
/// the opposite (180 deg about a perpendicular). \a twist_rad is an extra
/// rotation about the fixed axis after coaxial placement (clock rotation).
